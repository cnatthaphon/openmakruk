...
- **🎬 Bot Exhibition** — an external runner (`scripts/exhibition-runner.mjs`, scheduled via GitHub Actions) picks two bots every 30 minutes, plays them against each other using full Fairy-Stockfish via ffish-es6, and submits the game through an admin-gated API endpoint. The platform stays alive with fresh content even when no users are online. Public feed + step-through replay viewer + tier filter chips. (Earlier versions ran the picker inside the Cloudflare Worker cron; we moved it out so the games use the same engine strength the browser does — see [external bot architecture](#bot-exhibition-external-runner-architecture) below.)
