...
[![CI](https://github.com/cnatthaphon/openmakruk/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/cnatthaphon/openmakruk/actions/workflows/ci.yml)
...
- **🎬 Bot Exhibition** — an external runner (`scripts/exhibition-runner.mjs`, scheduled every 30 min via [`.github/workflows/exhibition-tick.yml`](.github/workflows/exhibition-tick.yml)) picks two bots, plays them against each other using full Fairy-Stockfish via ffish-es6, and submits the game through an admin-token-gated API endpoint. The platform stays alive with fresh content even when no users are online. Public feed + step-through replay viewer + tier filter chips. (Earlier versions ran the picker inside the Cloudflare Worker cron; we moved it out so the games use the same engine strength the browser does.)
...
<td width="33%">
  ...
  <p align="center"><sub>🎬 Bot Exhibition — bot-vs-bot games generated by a scheduled GitHub Actions runner</sub></p>
</td>
</tr>
</table>
...
│  • Exhibition submit endpoint (admin-token gated) — receives    │
│    bot-vs-bot games from the external GitHub Actions runner     │
│    every 30 min and stores them for the public feed             │
...
```bash
npm run test:core                 # Node `node:test` — pure-TS core (FEN, counting, …)
npm run test:e2e                  # Playwright — every primary route + key flows
cd worker && npm run test         # Vitest — worker integration suite
```

Exact counts vary as suites grow. The CI badge above tracks main-branch green/red across all three.
...
| Worker engine | Pure-JS Makruk rules for move verification (exhibition games are generated externally — see Bot Exhibition above) |
