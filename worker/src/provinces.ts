// Thailand province catalog + region rollup.
//
// Codes are ISO 3166-2:TH (e.g. TH-10 = Bangkok, TH-50 = Chiang Mai).
// We strip the "TH-" prefix to keep stored values short (the table
// only contains Thai provinces so the country prefix is redundant).
//
// Region rollup uses the 6-region classification preferred by the
// Thai government's geography curriculum:
//   north         (ภาคเหนือ)         — 9 provinces
//   northeast     (ภาคตะวันออกเฉียงเหนือ / อีสาน) — 20 provinces
//   central       (ภาคกลาง)         — 22 provinces (includes Bangkok)
//   east          (ภาคตะวันออก)      — 7 provinces
//   west          (ภาคตะวันตก)       — 5 provinces
//   south         (ภาคใต้)           — 14 provinces
//
// Total: 77 (76 จังหวัด + 1 กทม.)
//
// The data is intentionally a flat array so both client and worker
// can ship the same source without converting shape.
...
export type Province = {
  /** ISO 3166-2:TH suffix — e.g. "10" for Bangkok. Stable. */
  code: string;
  /** Thai name. */
  nameTh: string;
  /** English transliteration for fallback UI. */
  nameEn: string;
  region: Region;
};
...
export const PROVINCES: Province[] = [
  ...
  { code: '20', nameTh: 'ชลบุรี',        nameEn: 'Chonburi',       region: 'east' }, // sometimes central
  ...
  { code: '63', nameTh: 'ตาก',           nameEn: 'Tak',            region: 'west' },   // sometimes northern
  ...
  // ภาคตะวันออก (7) — using common 7-province classification
...
/** Look up a province by its short code (e.g. "10" for Bangkok). */
export function findProvince(code: string | null | undefined): Province | null {
  if (!code) return null;
  return BY_CODE.get(code) ?? null;
}

/** Resolve a province code to its region. Unknown / null → null. */
export function regionOf(code: string | null | undefined): Region | null {
  const p = findProvince(code);
  return p ? p.region : null;
}

/** Sorted-by-name (Thai) version for picker UIs. Memoized at module-
 *  evaluation time — the array is tiny so the cost is one-shot. */
export const PROVINCES_SORTED_TH = [...PROVINCES].sort((a, b) =>
  a.nameTh.localeCompare(b.nameTh, 'th'),
);

/** Group provinces by region for sectioned dropdowns. */
...
/** Validate a province code came from the canonical list (server-side
 *  gate before persisting — prevents arbitrary client-supplied junk
